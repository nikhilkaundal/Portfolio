...
import ByteDefender from "../hero/ByteDefender";
  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      tl.to(".hero-line", { opacity: 1, scaleX: 1, duration: 0.6, ease: "power3.out" })
        ...
        .to(".hero-game-card", { opacity: 1, y: 0, duration: 0.6, ease: "power3.out" }, "-=0.4")
        .to(".hero-scroll", { opacity: 1, duration: 0.4 }, "-=0.2");
    }, containerRef);

    // Guaranteed fallback: ensures text elements are 100% visible even if GSAP timeline is interrupted
    const fallbackTimer = setTimeout(() => {
      if (containerRef.current) {
        const els = containerRef.current.querySelectorAll(".hero-line, .hero-eyebrow, .hero-n1, .hero-n2, .hero-sub, .hero-ctas, .hero-side, .hero-game-card, .hero-scroll");
        els.forEach((el) => {
  ...
  return (
    <section id="hero" ref={containerRef} className={sectionClass}>
      ...
      <div className="absolute inset-0 w-full h-full pointer-events-none z-0">
        ...
        {/* Subtle Ambient Amber Glow Orbs (Soft dark tone, no bright white spot) */}
        <motion.div
          animate={{ scale: [1, 1.15, 1], opacity: [0.2, 0.35, 0.2] }}
          transition={{ duration: 8, repeat: Infinity, ease: "easeInOut" }}
          className="absolute top-[15%] left-[10%] w-[350px] sm:w-[500px] h-[350px] sm:h-[500px] rounded-full bg-amber-500/15 blur-[120px] pointer-events-none z-0"
        />
        <motion.div
          animate={{ scale: [1, 1.2, 1], opacity: [0.15, 0.3, 0.15] }}
          transition={{ duration: 10, repeat: Infinity, ease: "easeInOut", delay: 2 }}
          className="absolute bottom-[15%] right-[15%] w-[350px] sm:w-[500px] h-[350px] sm:h-[500px] rounded-full bg-amber-600/10 blur-[130px] pointer-events-none z-0"
        />

        {/* Seamless Dark Vignette (Centered, uniform dark background) */}
        <div
          className="absolute inset-0 pointer-events-none z-0"
          style={{
            background:
              "radial-gradient(ellipse 80% 80% at 50% 50%, rgba(var(--night-rgb),0.4) 0%, rgba(var(--night-rgb),0.92) 70%, rgba(var(--night-rgb),0.99) 100%)",
          }}
        />
      </div>

      {/* Primary layout container — split layout on desktop */}
      <div className="flex flex-col lg:flex-row items-center justify-center gap-8 lg:gap-14 w-full max-w-6xl mx-auto relative z-10">
        {/* Left: Main Text Content */}
        <motion.div
          className="w-full lg:w-1/2 select-none relative z-10 pointer-events-auto flex flex-col justify-center items-center lg:items-start text-center lg:text-left py-4 sm:py-6 lg:py-0"
        >
          {/* Decorative line */}
          <div
          ...
          {/* Subtitle */}
          <p
            className="hero-sub font-body font-medium text-bark/70 tracking-[0.12em] sm:tracking-[0.14em] uppercase mt-4 sm:mt-5 text-[0.68rem] sm:text-xs max-w-md leading-relaxed"
            style={{ opacity: 0 }}
          >
        ...
        >
          ...
          {/* Desktop View (>= lg): Full Inline Arcade Card */}
          <div className="hidden lg:block byte-defender-card rounded-xl border border-bark/15 bg-surface/90 dark:bg-night/80 backdrop-blur-sm shadow-[0_0_40px_rgba(192,88,0,0.06)] overflow-hidden transition-colors duration-300">
            {/* Card header with Console Logo */}
            <div className="px-4 py-2.5 border-b border-bark/15 flex items-center justify-between bg-surface-dark/40 dark:bg-surface/30 transition-colors duration-300">
              <div className="flex items-center gap-2">
                <Gamepad2 className="w-4 h-4 text-amber" />
                <span className="font-mono text-[0.6rem] sm:text-[0.65rem] tracking-[0.14em] uppercase text-bark font-bold">
                  BYTE DEFENDER <span className="text-amber font-semibold ml-1">v5.5</span>
                </span>
              </div>
              <span className="font-mono text-[0.52rem] uppercase tracking-wider text-amber bg-amber/10 border border-amber/20 px-2 py-0.5 rounded font-semibold">
                ARCADE 🎮
              </span>
            </div>
      ...
      {/* Floating Arcade Window Modal Overlay for Mobile */}
      <AnimatePresence>
        {isMobileGameOpen && (
          <div className="fixed inset-0 z-[99999] flex items-center justify-center bg-white/40 dark:bg-black/60 backdrop-blur-md p-3 sm:p-5 transition-colors duration-300">
            <motion.div
              initial={{ opacity: 0, scale: 0.95, y: 15 }}
              animate={{ opacity: 1, scale: 1, y: 0 }}
              exit={{ opacity: 0, scale: 0.95, y: 15 }}
              transition={{ duration: 0.25, ease: "easeOut" }}
              className="w-full max-w-lg h-[84vh] rounded-2xl border border-amber/35 bg-surface/95 dark:bg-night/95 shadow-2xl flex flex-col overflow-hidden relative backdrop-blur-lg"
            >
              {/* Floating Modal Header with Close (X) Sign */}
              <div className="px-4 py-3 border-b border-bark/15 flex items-center justify-between bg-surface-dark/80 dark:bg-surface/50 select-none shrink-0">
                <div className="flex items-center gap-2">
                  <Gamepad2 className="w-4 h-4 text-amber" />
                  <span className="font-mono text-xs tracking-wider uppercase text-bark font-bold">
                    BYTE DEFENDER <span className="text-amber">v5.5</span>
                  </span>
                </div>

                {/* Prominent Close (X) Sign Button */}
                <button
                  onClick={() => setIsMobileGameOpen(false)}
                  className="p-1.5 rounded-full bg-bark/10 hover:bg-red-500/20 text-bark hover:text-red-500 transition-colors cursor-pointer flex items-center justify-center"
              ...
              {/* Floating Game Canvas (Fills 100% of remaining area flush to all edges) */}
