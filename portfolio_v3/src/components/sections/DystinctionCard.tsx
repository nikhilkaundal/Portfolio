...
const ModuleRow: React.FC<{ mod: Module; index: number; isOpen: boolean; onToggle: () => void }> = ({
  mod,
  index,
  isOpen,
  onToggle,
  ...
  >
    ...
    >
      {/* Numbered index badge — smooth rounded square */}
      <span
    ...
    >
      ...
      <div className="relative p-5 sm:p-8 lg:p-10">
        ...
        <div className="flex flex-col lg:flex-row lg:items-start gap-6 mb-8">
          ...
          <div className="grid grid-cols-3 lg:flex lg:flex-col gap-3 w-full lg:w-auto flex-shrink-0">
            {exp.metrics.map((m) => (
              <div
                key={m.label}
                className="border border-amber/20 bg-surface/80 rounded-2xl px-2 py-2.5 sm:px-4 sm:py-3 text-center
        ...
        <div className="mb-8">
          ...
          <ul className="space-y-3">
            {exp.highlights.map((h, i) => (
              <li
                key={i}
                className="font-body text-[0.875rem] text-bark/65 leading-relaxed
                  flex gap-3"
              >
        ...
        <div>
          ...
          {/* Smooth rounded accordion container — removes harsh white outline */}
